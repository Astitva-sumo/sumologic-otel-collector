...
echo "$msg"
FROM_VER="$(echo "$msg" | sed -n 's/.*deps(otel): Updated opentelemetry-collector from v\([0-9.]*\) to v[0-9.]*.*/\1/p')"
TO_VER="$(echo "$msg" | sed -n 's/.*deps(otel): Updated opentelemetry-collector from v[0-9.]* to v\([0-9.]*\).*/\1/p')"
